...
class Frontend {
	...
	public function comment_form($args) {
		...
		$args['fields'] = [];
		if( $author ){
			$args['fields']['author'] = '<div class="rtrs-form-group"><input id="name" class="rtrs-form-control" placeholder="' . esc_html__('Name', 'review-schema') . ( $req ? "*" : '') .'" name="author"  type="text" value="' . esc_attr($commenter['comment_author']) . '" size="30"' . ($req ? " required='required' aria-required='true'" : '') . ' /></div>';
		}
		if( $email ){
			$args['fields']['email'] = '<div class="rtrs-form-group"><input id="email" class="rtrs-form-control" placeholder="' . esc_html__('Email', 'review-schema') . ( $req ? "*" : '') . '" name="email"  type="text" value="' . esc_attr($commenter['comment_author_email']) . '" size="30"' . ($req ? " required='required' aria-required='true'" : '') . ' /></div>';
		}

		if( $website ){
			$args['fields']['url'] = '<div class="rtrs-form-group"><input id="url" class="rtrs-form-control" placeholder="' . esc_html__('Website', 'review-schema') . '" name="url" type="text" value="' . esc_url($commenter['comment_author_url']) . '" size="30"/></div>';
		}
		...
		$args['title_reply']        = esc_html__('Leave feedback about this', 'review-schema');
		...
		$args['title_reply_to']       = wp_kses(__('Leave feedback about this to %s', 'review-schema'), ['allow_tag_list']);
		$args['cancel_reply_link']    = esc_html__('Cancel Reply', 'review-schema');
		$args['comment_notes_before'] = '';
		$args['comment_notes_after']  = '';
		$args['label_submit']         = esc_html__('Submit Review', 'review-schema');

		$args['comment_field'] = '';
		
		if ($title) {
			$args['comment_field'] = '<div class="rtrs-form-group rtrs-hide-reply"><input id="rt_title" class="rtrs-form-control" placeholder="' . esc_html__('Title', 'review-schema') . '" name="rt_title" type="text" value="" size="30" aria-required="true"></div>';
		}

		$args['comment_field'] .= '<div class="rtrs-form-group"><textarea id="message" class="rtrs-form-control" placeholder="' . esc_html__('Write your review', 'review-schema') . '* "  name="comment" required="required"  aria-required="true" rows="6" cols="45"></textarea></div>';
