...
<div class="rtrs-review-wrap <?php echo esc_attr( $parent_class ); ?> rtrs-review-post-type-<?php echo esc_attr( $get_post_type ); ?> rtrs-review-sc-<?php echo esc_attr( $p_meta['sc_id'] ); ?>" id="comments">
    <?php if ( have_comments() ) : ?> 
        <?php 
            ...
            if ( $total_rating ) {
                Functions::get_template_part( 'summary/layout-' . $layout, 
                    array(
        ...
        <div class="rtrs-sorting-bar">
            <h3 class="rtrs-sorting-title"> 
                <?php 
                    printf(
                        esc_html( _n( 'Reviewed by %s customer', 'Reviewed by %s customers', $total_rating, 'review-schema' ) ), 
        ...
        if ( $pagination_type == "number" ) { ?>
            ...
            <div class="rtrs-paginate">
                <?php paginate_comments_links( array('prev_text' => '<i class="rtrs-angle-left"></i>', 'next_text' => '<i class="rtrs-angle-right"></i>' ) ); ?>
            </div>
            <?php } ?>
        <?php } else if ( $pagination_type == "number-ajax" ) { ?>
            <?php if ( get_the_comments_pagination() ) { ?>
            <div class="rtrs-paginate rtrs-paginate-ajax" data-max="<?php echo esc_attr( get_comment_pages_count() ); ?>">
                <?php paginate_comments_links( array('prev_text' => '<i class="rtrs-angle-left"></i>', 'next_text' => '<i class="rtrs-angle-right"></i>' ) ); ?>
            </div>
            <?php } ?>
        <?php } else if ( $pagination_type == "load-more" ) { ?>
        ...
        <?php } else if ( $pagination_type == "auto-scroll" ) { ?>
