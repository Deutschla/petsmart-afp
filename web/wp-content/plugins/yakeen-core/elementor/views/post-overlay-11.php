				...
				<div class="rt-item rt-mask-content rt-text-light rt-single-item">					
					<div class="rt-image">
						<?php if ( ( $data['post_video'] == 'yes' && 'video' == get_post_format( $id ) && !empty( $youtube_link ) ) ) { ?>
							<a class="rt-play play-btn-lg btn-position-top-right rt-video-popup" href="<?php echo esc_url( $youtube_link );?>"><i class="fas fa-play"></i></a>
						<?php } ?>
							...
							<?php
								if ( has_post_thumbnail() ){
									...
									if ( !empty( YakeenTheme::$options['no_preview_image']['id'] ) ) {
										...
										echo '<img class="wp-post-image" src="' . YakeenTheme_Helper::get_img( 'noimage_1110X550.jpg' ) . '" alt="'.get_the_title().'">';
									}
								}
							?>
						</a>										
