...
$yakeen_has_entry_meta1  = ( $data['post_author'] == 'yes' || $data['post_date'] == 'yes' || $data['post_comment'] == 'yes' || $data['post_length'] == 'yes' && function_exists( 'yakeen_reading_time' ) || $data['post_view'] == 'yes' && function_exists( 'yakeen_views' ) ) ? true : false;

$yakeen_has_entry_meta2  = ( $data['small_post_author'] == 'yes' || $data['small_post_date'] == 'yes' || $data['small_post_comment'] == 'yes' || $data['small_post_length'] == 'yes' && function_exists( 'yakeen_reading_time' ) || $data['small_post_view'] == 'yes' && function_exists( 'yakeen_views' ) ) ? true : false;

$thumb_size1 = 'yakeen-size5';
$thumb_size2 = 'yakeen-size9';
			...
			<?php if ( $count == 1 ) { ?>
				<div class="col-xl-7 col-lg-6">
					<div class="rt-item-wrap <?php echo esc_attr( $data['animation'] );?> <?php echo esc_attr( $data['animation_effect'] );?>" data-wow-delay="<?php echo esc_attr( $i );?>s" data-wow-duration="<?php echo esc_attr( $j );?>s">
						<div class="rt-item rt-mask-content rt-single-item">
							<div class="rt-image">
								<?php if ( ( $data['post_video'] == 'yes' && 'video' == get_post_format( $id ) && !empty( $youtube_link ) ) ) { ?>
									<div class="rt-video"><a class="rt-play play-btn-lg btn-position-top-left rt-video-popup" href="<?php echo esc_url( $youtube_link );?>"><i class="fas fa-play"></i></a></div>
								<?php } ?>
									...
									<?php
										if ( has_post_thumbnail() ){
											...
											if ( !empty( YakeenTheme::$options['no_preview_image']['id'] ) ) {
												echo wp_get_attachment_image( YakeenTheme::$options['no_preview_image']['id'], $thumb_size1 );
											}
											else {
												echo '<img class="wp-post-image" src="' . YakeenTheme_Helper::get_img( 'noimage_732X576.jpg' ) . '" alt="'.get_the_title().'">';
											}
										}
									?>
								</a>										
							...
							<div class="entry-content">
								...
								<?php } ?>	
								<h3 class="entry-title title-size-xl title-animation-underline size-2"><a href="<?php the_permalink();?>"><?php echo esc_html( $title );?></a></h3>
								<?php if ( $yakeen_has_entry_meta1 ) { ?>
                                    <ul class="entry-meta">
			...
			<?php } ?>
				...
				<?php if ( $count > 1 ) { ?>
					<div class="col-12 <?php echo esc_attr( $data['animation'] );?> <?php echo esc_attr( $data['animation_effect'] );?>" data-wow-delay="<?php echo esc_attr( $i );?>s" data-wow-duration="<?php echo esc_attr( $j );?>s">
						<div class="rt-item rt-multi-items">
							<div class="rt-image">
								<?php if ( ( $data['small_post_video'] == 'yes' && 'video' == get_post_format( $id ) && !empty( $youtube_link ) ) ) { ?>
									<div class="rt-video"><a class="rt-play play-btn-sm btn-position-center rt-video-popup" href="<?php echo esc_url( $youtube_link );?>"><i class="fas fa-play"></i></a></div>
								<?php } ?>
								<a href="<?php the_permalink(); ?>">
									<?php
										if ( has_post_thumbnail() ){
											...
											if ( !empty( YakeenTheme::$options['no_preview_image']['id'] ) ) {
												echo wp_get_attachment_image( YakeenTheme::$options['no_preview_image']['id'], $thumb_size2 );
											}
											else {
							...
							<div class="entry-content">
								<h3 class="entry-title title-animation-underline size-1"><a href="<?php the_permalink();?>"><?php echo esc_html( $small_title );?></a></h3>
								<?php if ( $yakeen_has_entry_meta2 ) { ?>
                                    <ul class="entry-meta">
										...
										<li class="post-comment"><i class="far fa-comments"></i><a href="<?php echo get_comments_link( get_the_ID() ); ?>"><?php echo wp_kses( $yakeen_comments_html , 'alltext_allow' );?></a></li>
										<?php } if ( ( $data['small_post_length'] == 'yes' ) && function_exists( 'yakeen_reading_time' ) ) { ?>
										<li class="post-reading-time meta-item"><?php echo yakeen_meta_icon_clock(); ?><?php echo yakeen_reading_time(); ?></li>
										<?php } if ( ( $data['small_post_view'] == 'yes' ) && function_exists( 'yakeen_views' ) ) { ?>
										<li><span class="post-views meta-item"><i class="fas fa-signal"></i><?php echo yakeen_views(); ?></span></li>
										<?php } ?>
									</ul>
								<?php } ?>
