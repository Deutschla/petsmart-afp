...
$slideper_view = $data['slideper_view']['size'];
...
?>
<div class="rt-post-slider-default rt-swiper-slider rt-post-slider-<?php echo esc_attr( $data['style'] );?>">
	<div class="swiper-container" data-xld ="<?php echo esc_attr( $data['swiper_data'] );?>">	
		<div class="swiper-wrapper">
			<?php $i = $data['delay']; $j = $data['duration'];
			$query = get_posts($args);
			$chunks = array_chunk($query, $slideper_view);
			foreach ($chunks as $row) : ?>
				<div class="swiper-slide">
					<?php 
					global $post; 
					foreach ($row as $key => $post) : setup_postdata($post); 
						...
						?>
						<?php if ( $key == 0 ) { ?>
						<div class="rt-item first-post rt-single-item <?php echo esc_attr( $data['animation'] );?> <?php echo esc_attr( $data['animation_effect'] );?>" data-wow-delay="<?php echo esc_attr( $i );?>s" data-wow-duration="<?php echo esc_attr( $j );?>s">
							<div class="rt-image">
								<?php if ( ( $data['post_video'] == 'yes' && 'video' == get_post_format( $id ) && !empty( $youtube_link ) ) ) { ?>
									<div class="rt-video"><a class="rt-play play-btn-transparent-2 rt-video-popup" href="<?php echo esc_url( $youtube_link );?>"><i class="fas fa-play"></i></a></div>
								<?php } ?>
		...
		<?php require YAKEEN_CORE_BASE_DIR .'elementor/views/slider-control.php'; ?>	
	</div>
	<?php wp_reset_query();?>
</div> 