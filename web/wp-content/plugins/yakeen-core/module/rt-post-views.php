...
if( ! class_exists( 'RT_POST_VIEWS' )){
	...
	class RT_POST_VIEWS{
		...
		function _set_post_views(){
			...
			if( !YakeenTheme::$options['post_view'] || ! is_single() ){
				 return;
			}
			...
			/*if( !YakeenTheme::$options['post_view'] || $page > 1  ){
				return false;
			}*/
		...
		function _posts_column_views( $defaults ){
			...
			if( YakeenTheme::$options['post_view'] ){
				$defaults['post_view'] = esc_html__( 'Views', 'yakeen-core' );
			}
...
/*----------------------------------------------------------*/
if( !function_exists( 'yakeen_views' )){

	function yakeen_views( $text = '', $post_id = 0 ){
		...
		if ( $view_count == 1 ) {
			$yakeen_view_html = esc_html__( 'View' , 'yakeen-core' );
		} else {
			$yakeen_view_html = esc_html__( 'Views' , 'yakeen-core' );
		}
		
		$yakeen_views_html = '<span class="view-number" >' . $view_count . '</span> ' . $yakeen_view_html;

		return '<span class="meta-views meta-item '. $views_class .'">'. $yakeen_views_html.'</span> ';
	}
}
