...
/* Post reading time */
if( !function_exists( 'yakeen_reading_time' )){

	function yakeen_reading_time(){
		...
		if( $reading_time < 1){
			$result = esc_html ( 'Less than a minute', 'yakeen-core' );
		}
		elseif( $reading_time > 60 ){
			$result = sprintf( esc_html( '%s hours read' ), floor( $reading_time / 60 ) );
		}
		else if ( $reading_time == 1 ){
			$result = esc_html( '1 minute read' );
		} else {
			$result = sprintf( esc_html( '%s minutes read' ), $reading_time );
		}
