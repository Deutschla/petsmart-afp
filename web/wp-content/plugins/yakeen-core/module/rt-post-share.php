	...
	if( get_post_type() != 'page' ) { 
		...
		$share_buttons = array(
			...
			'facebook-f' => array(
				...
				'text' => esc_html__( 'Facebook', 'yakeen-core' ),
			),

			'twitter' => array(
				'url'   => 'https://twitter.com/intent/tweet?text='. $post_title .'&amp;url='. $post_link, 
				'text'  => esc_html__( 'Twitter', 'yakeen-core' ),
			),

			'youtube' => array(
				'url'   => 'https://www.youtube.com?text='. $post_title .'&amp;url='. $post_link, 
				'text'  => esc_html__( 'Youtube', 'yakeen-core' ),
			),

			'linkedin-in' => array(
				'url'   => 'http://www.linkedin.com/shareArticle?mini=true&amp;url='. $post_link .'&amp;title='. $post_title,
				'text'  => esc_html__( 'LinkedIn', 'yakeen-core' ),
			),
			
			'pinterest' => array(
				'url'   => 'http://pinterest.com/pin/create/button/?url='. $post_link .'&amp;description='. $post_title .'&amp;media='. yakeen_post_img_src( 'yakeen-size1' ),
				'text'  => esc_html__( 'Pinterest', 'yakeen-core' ),
			),
			
			'whatsapp' => array(
				'url'   => 'https://api.whatsapp.com/send?text='. $post_title . ' – ' . $post_link ,
				'text'  => esc_html__( 'Whatsapp', 'yakeen-core' ),
			),

			'cloud' => array(
				'url'   => 'https://cloud.google.com/send?text='. $post_title . ' – ' . $post_link ,
				'text'  => esc_html__( 'Cloud', 'yakeen-core' ),
				'icon'  => 'fas fa-cloud',
			),

			'dribbble' => array(
				'url'   => 'https://dribbble.com?url='. $post_link .'&amp;title='. $post_title,
				'text'  => esc_html__( 'StumbleUpon', 'yakeen-core' ),
			),

			'tumblr' => array(
				'url'   => 'http://www.tumblr.com/share/link?url='. $post_link .'&amp;name='. $post_title,
				'text'  => esc_html__( 'Tumblr', 'yakeen-core' ),
			),

			'reddit' => array(
				'url'   => 'http://reddit.com/submit?url='. $post_link .'&amp;title='. $post_title,
				'text'  => esc_html__( 'Reddit', 'yakeen-core' ),
			),			
			
			'print' => array(
				'url'   => '#',
				'text'  => esc_html__( 'Print', 'yakeen-core' ),
				'icon'  => 'fas fa-print',
			),

			'email' => array(
				'url'   => 'mailto:?subject='. $post_title .'&amp;body='. $post_link,
				'text'  => esc_html__( 'Share via Email' , 'yakeen-core' ),
				'icon'  => 'far fa-envelope',
			),
		);
		
		if ( YakeenTheme::$options['post_share_facebook'] == 0 ){
		unset($share_buttons['facebook-f']);
		}
		if ( YakeenTheme::$options['post_share_twitter'] == 0 ){
		unset($share_buttons['twitter']);
		}
		if ( YakeenTheme::$options['post_share_youtube'] == 0 ){
		unset($share_buttons['youtube']);
		}
		if ( YakeenTheme::$options['post_share_linkedin'] == 0 ){
		unset($share_buttons['linkedin-in']);
		}
		if ( YakeenTheme::$options['post_share_pinterest'] == 0 ){
		unset($share_buttons['pinterest']);
		}
		if ( YakeenTheme::$options['post_share_whatsapp'] == 0 ){
		unset($share_buttons['whatsapp']);
		}
		if ( YakeenTheme::$options['post_share_cloud'] == 0 ){
		unset($share_buttons['cloud']);
		}
		if ( YakeenTheme::$options['post_share_dribbble'] == 0){
		unset($share_buttons['dribbble']);
		}
		if ( YakeenTheme::$options['post_share_tumblr'] == 0 ){
		unset($share_buttons['tumblr']);
		}
		if ( YakeenTheme::$options['post_share_reddit'] == 0 ){
		unset($share_buttons['reddit']);
		}
		if ( YakeenTheme::$options['post_share_print'] == 0 ){
		unset($share_buttons['print']);
		}
		if ( YakeenTheme::$options['post_share_email'] == 0 ){
