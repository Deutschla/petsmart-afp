...
// Logo
if( !empty( YakeenTheme::$options['footer2_logo_light'] ) ) {
	$logo_lights = wp_get_attachment_image( YakeenTheme::$options['footer2_logo_light'], 'full' );
...
if( !empty( YakeenTheme::$options['fbgimg2'] ) ) {
	$f1_bg = wp_get_attachment_image_src( YakeenTheme::$options['fbgimg2'], 'full', true );
	$footer_bg_img = $f1_bg[0];
}else {
	$footer_bg_img = YAKEEN_IMG_URL . 'footer4_bg.png';
}

if ( YakeenTheme::$options['footer_bgtype2'] == 'fbgcolor2' ) {
	$yakeen_bg = YakeenTheme::$options['fbgcolor2'];
} else {
	$yakeen_bg = 'url(' . $footer_bg_img . ') no-repeat center bottom / cover';
}
$bgc = '';
if ( YakeenTheme::$options['footer_bgtype2'] == 'fbgimg2' ) {
	$bgc = 'footer-bg-opacity';
}
if( YakeenTheme::$options['footer2_social'] == '1' ) {
...
<?php if ( YakeenTheme::$footer_area == 1 || YakeenTheme::$copyright_area == 1 ) { ?>
	<div class="footer-top-area <?php echo esc_attr( $bgc ); ?>" style="background:<?php echo esc_html( $yakeen_bg ); ?>">	
		...
		<?php if ( YakeenTheme::$options['footer2_animate_shape'] == 1 ) { ?>
		...
		<div class="container">
			<div class="footer-main-content">
				<?php if ( YakeenTheme::$footer_area == 1  ) { ?>
					<?php if ( YakeenTheme::$options['footer2_logo'] ) { ?>
			...
			<div class="footer-bottom-content <?php echo esc_attr($preview_social); ?> <?php echo esc_attr($preview_copyright); ?>">
				...
				<?php }  } ?>			
				<?php if (YakeenTheme::$options['footer2_social']) { ?>
				<?php if ($yakeen_socials && (YakeenTheme::$options['footer2_social'])) { ?>
