...
if (  YakeenTheme::$layout == 'right-sidebar' || YakeenTheme::$layout == 'left-sidebar' ){
	$post_classes = array( 'col-12 rt-grid-item blog-layout-2 ' . $wow . ' ' . $effect );
	$ul_class = 'side_bar';
} else {
	$post_classes = array( 'col-md-10 col-12 mx-auto rt-grid-item blog-layout-2 ' . $wow . ' ' . $effect );
	$ul_class = '';
}
					...
					<?php } else {
						if ( YakeenTheme::$options['display_no_preview_image'] == '1' ) {
							...
							elseif ( empty( YakeenTheme::$options['no_preview_image']['id'] ) ) {
								$thumbnail = '<img class="wp-post-image" src="'.YAKEEN_IMG_URL.'noimage_1110X550.jpg" alt="'. the_title_attribute( array( 'echo'=> false ) ) .'">';
							}
